...
1. Create a Firebase project, enable Firestore, deploy `firestore.rules`
2. Set `DATA_STORE=firestore` and `FIREBASE_ADMIN_*` + `NEXT_PUBLIC_FIREBASE_*` on Vercel
3. Deploy to Vercel; run `POST /api/recovery/reseed` once
4. Without Firebase, `DATA_STORE=memory` works for a single-server demo (state resets on cold start)
